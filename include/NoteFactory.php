...
class NoteFactory
{
	...
	public static function getNotesByNotes($tag){
		$db = DatabaseConnectionFactory::getConnection();
		$notes = array();

		$query = "SELECT * FROM note WHERE details LIKE '%{$tag}%'";
	...
	public static function getNoteNumsByMonth(){
		$db = DatabaseConnectionFactory::getConnection();
		$noteNums = array();

		$query = "SELECT DATE_FORMAT(recorded, '%Y') as 'year',
					DATE_FORMAT(recorded, '%m') as 'month',
					COUNT(id) as 'total',
					id
					FROM note
					GROUP BY DATE_FORMAT(recorded, '%Y%m')";
		...
		return json_encode($noteNums);
	}
	
	public static function getUserNumsByNote(){
		$db = DatabaseConnectionFactory::getConnection();
		$noteNums = array();
	...
	public static function getNoteTypeNumsByCategory(){
		$db = DatabaseConnectionFactory::getConnection();
		$noteTypeNums = array();

		$query = "SELECT note_type,
					COUNT(id) as 'total'
					FROM note
					GROUP BY note_type";
		...
		return json_encode($noteTypeNums);
	}
