...
if ( $version == PROTOCOL_VERSION_1 || $version == PROTOCOL_VERSION_2 || $version == null) {
  ...
  $cotwo    = isset( $_POST['cotwo'] )   ? $_POST['cotwo']   : null; 
  $kcal     = isset( $_POST['kcal'] )    ? $_POST['kcal']    : null; 
  $avgCost  = isset( $_POST['avgcost'] ) ? $_POST['avgcost'] : null; 
...
elseif ( $version == PROTOCOL_VERSION_3) {
  ...
  $distance = isset( $query_vars['distance'] )? $query_vars['distance']: null; 
  $cotwo    = isset( $query_vars['cotwo'] )   ? $query_vars['cotwo']   : null; 
  $kcal     = isset( $query_vars['kcal'] )    ? $query_vars['kcal']    : null; 
  $avgCost  = isset( $query_vars['avgcost'] ) ? $query_vars['avgcost'] : null; 
  $score    = isset( $query_vars['score'] )   ? $query_vars['score']   : null; 
}
...
{
	Util::log ( "****Distance: {$distance}****, ****CO2: {$cotwo}****, ****KCal: {$kcal}****, ****Avg Cost: {$avgCost}****, ****Score: {$score}****" );
	...
	if ( $user )
	{
		...
		// add a trip
			...
			if ( $userObj  = new User( $userData ) )
			{
				...
				//Util::log( ":::::LOOK HERE:::::: User: { $user }, User Object: { $userData } ");
				Util::log( ", Test userID: { $user->id }, Test userAgreement: { $user->agree } ");
				Util::log( ", Test newUserId: { $userData->id }, Test newUserAgree: { $userData->agree }" );	
				// update user record
			...
			if ( $trip = TripFactory::insert( $user->id, $purpose, $notes, $start, $distance, $cotwo, $kcal, $avgCost, $score ) )
			{
				// This is unique to Fountain City.  These save locally on the phone AND get to database.
				//TripFactory::insertExtras( $distance, $cotwo, $kcal, $avgCost);
				TripFactory::updateLeaderboard( $user->id, $score, $distance, $device );

				$coord = null;
				if ( $version == PROTOCOL_VERSION_3 ){					
					$stop = CoordFactory::insert_bulk( $trip->id, $user->id, $coords );	
				}
				else if ( $version == PROTOCOL_VERSION_2 ){
					$stop = CoordFactory::insert_bulk_protocol_2 ( $trip->id, $user->id, $coords );
